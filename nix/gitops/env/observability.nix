...
let
  ...
  # a CH replica — writes go via the clickhouse-local Service
  ...
  collectorValues = ''
    ...
    # replica on the same host (writes via clickhouse-local Service).
    affinity:
      nodeAffinity:
...
in
{
  manifests = [
    ...
    # ─── CH user-creation Job ────────────────────────────────────────
    # sync-wave 1: runs alongside the schema-bootstrap Job. Creates the
    # `otel` (writer) and `hyperdx` (reader) CH users using passwords
    # from the otel-ch-credentials and clickstack-hyperdx-config Secrets
    # pre-generated by nix/secrets.nix. If the Secrets still hold
    # placeholder values the Job fails and retries (backoffLimit: 10)
    # until real credentials arrive.
    #
    # All DDL uses `IF NOT EXISTS` / `ALTER USER` so re-runs are safe.
    {
      name = "observability/job-ch-users.yaml";
      content = ''
        apiVersion: batch/v1
        kind: Job
        metadata:
          ...
          annotations:
            ...
            argocd.argoproj.io/hook-delete-policy: BeforeHookCreation
          labels:
            app.kubernetes.io/name: otel-ch-users
            app.kubernetes.io/part-of: observability
        spec:
          backoffLimit: 10
          template:
              ...
              - name: clickhouse-client
                ...
                command:
                  ...
                  - |
                    ...
                    CREATE USER IF NOT EXISTS ${ch.user} ON CLUSTER ${ch.cluster} IDENTIFIED BY '$CLICKHOUSE_PASSWORD';
                    ALTER  USER ${ch.user} ON CLUSTER ${ch.cluster} IDENTIFIED BY '$CLICKHOUSE_PASSWORD';
                    GRANT  INSERT ON ${ch.database}.* TO ${ch.user} ON CLUSTER ${ch.cluster};
                    GRANT  SELECT ON ${ch.database}.* TO ${ch.user} ON CLUSTER ${ch.cluster};

                    CREATE USER IF NOT EXISTS ${ch.uiUser} ON CLUSTER ${ch.cluster} IDENTIFIED BY '$HYPERDX_PASSWORD';
                    ALTER  USER ${ch.uiUser} ON CLUSTER ${ch.cluster} IDENTIFIED BY '$HYPERDX_PASSWORD';
                    GRANT  SELECT ON ${ch.database}.* TO ${ch.uiUser} ON CLUSTER ${ch.cluster};
                    SQL
                ...
                - name: HYPERDX_PASSWORD
                  valueFrom:
                    secretKeyRef:
                      name: otel-ch-credentials
                      key: HYPERDX_PASSWORD
                resources:
                  requests:
                    cpu: 50m
                    memory: 128Mi
