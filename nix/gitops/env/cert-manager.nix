      ...
      content = ''
        ...
        spec:
          ca:
            secretName: selfsigned-lab-ca-cert
        ---
        # ── Phase 2: Let's Encrypt DNS-01 via RFC2136 → PowerDNS ──────
        # Uses PowerDNS authoritative DNS (VIP ${constants.pdns.vip}) for
        # DNS-01 ACME challenges. TSIG key authenticates dynamic updates.
        # The pdns-tsig-secret Secret is pre-generated by nix/secrets.nix.
        apiVersion: cert-manager.io/v1
        kind: ClusterIssuer
        metadata:
        ...
        spec:
          acme:
            ...
            email: ${constants.pdns.acmeEmail}
            privateKeySecretRef:
              name: letsencrypt-prod-dns01-account-key
            solvers:
            - dns01:
                rfc2136:
                  nameserver: ${constants.pdns.vip}:${toString constants.pdns.dnsPort}
                  tsigKeyName: ${constants.pdns.tsigKeyName}
                  tsigAlgorithm: HMACSHA256
                  tsigSecretSecretRef:
                    name: pdns-tsig-secret
                    key: tsig-secret-key
              selector:
                dnsZones:
                ${lib.concatMapStringsSep "\n        " (d: "- \"${d}\"") constants.pdns.domains}
      '';
    }
